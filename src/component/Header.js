  ...
  return (
    <nav className="px-5 md:px-14 lg:px-32 xl:px-44 flex items-center justify-between h-16 lg:h-14 shadow-md bg-yellow-200">
      ...
      <div className="ml-auto lg:hidden z-50" onClick={handleToggle}>
        {toggle === true ? <FaBars size="1.5em" /> : <ImCross size="1.5em" />}
      </div>

      <div
        className={
          "lg:hidden absolute px-7 top-0 left-0 w-full h-screen flex backdrop-brightness-75 transition ease-in-out delay-150 z-40 hidden" +
          (toggle === true ? " block " : null)
        }
      >
